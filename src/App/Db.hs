...
import Data.Time.Calendar (diffDays, fromGregorian, Day)
...
initDB connStr = bracket (connectPostgreSQL connStr) close $ \conn -> do
  execute_ conn [sql|CREATE TABLE IF NOT EXISTS cases
      (date DATE NOT NULL PRIMARY KEY, new_cases INTEGER NOT NULL)|]
  execute_ conn [sql|CREATE TABLE IF NOT EXISTS example_scenario
      (date DATE NOT NULL PRIMARY KEY, new_cases INTEGER NOT NULL)|]
  executeMany conn [sql|INSERT INTO example_scenario (date, new_cases) VALUES (?, ?)
      ON CONFLICT (date) DO NOTHING|] extrapolatedScenario
  pass
...
updateDB conns = withResource conns $ \conn -> do
  ...
  case diff of
    ...
    _ -> do
      dateCases <- getCases
      executeMany conn [sql|INSERT INTO cases (date, new_cases) VALUES (?, ?)
          ON CONFLICT (date) DO UPDATE SET new_cases = excluded.new_cases|] dateCases
      pass
